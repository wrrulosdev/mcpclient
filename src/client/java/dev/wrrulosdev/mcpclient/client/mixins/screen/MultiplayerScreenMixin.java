...
public abstract class MultiplayerScreenMixin extends Screen {
    ...
    /**
     * <p>Initializes the multiplayer screen with a custom layout, server list loading, 
     * and network scanning for LAN servers. It replaces the original button setup 
     * with custom GUI components.</p>
     *
     * <p>Additionally, it ensures server list persistence, sets up LAN server detection, 
     * and defines buttons for connecting, editing, deleting, and adding servers.</p>
     */
    @Overwrite
    public void init() {
        if (this.initialized) {
            ...
            this.serverListWidget = new MultiplayerServerListWidget(
                (MultiplayerScreen) Mcpclient.getMinecraftClient().currentScreen,
                this.client,
                this.width,
                this.height - 64 - 32,
                32,
        ...
        this.buttonJoin = (ButtonWidget) this.addDrawableChild(
            ButtonWidget.builder(Text.translatable("selectServer.select"), (button) -> this.connect())
                .width(0)
                .build()
        );

        ButtonWidget buttonWidget = (ButtonWidget) this.addDrawableChild(
            ButtonWidget.builder(Text.translatable("selectServer.direct"), (button) -> {
        ...
        this.buttonEdit = (ButtonWidget) this.addDrawableChild(
            ButtonWidget.builder(Text.translatable("selectServer.edit"), (button) -> {
        ...
        this.buttonDelete = (ButtonWidget) this.addDrawableChild(
            ButtonWidget.builder(Text.translatable("selectServer.delete"), (button) -> {
                MultiplayerServerListWidget.Entry entry = (MultiplayerServerListWidget.Entry) this.serverListWidget.getSelectedOrNull();
                if (entry instanceof MultiplayerServerListWidget.ServerEntry) {
    ...
    /**
     * <p>Renders the screen, including the server list and all interactive buttons.</p>
     * <p>Each button uses custom textures that change when hovered:
     * delete, edit, refresh (sync), join, add, and spoof settings.</p>
     *
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        String deleteTexture = (currentEntry == null)
            ? "textures/gui/icons/delete_before.png"
            : ((mouseX >= deleteX && mouseX <= deleteX + (textureWidth - 2.5) && mouseY >= deleteY && mouseY <= deleteY + (textureHeight - 2.5))
            ...
            .onPress(button -> {
                ...
                if (entry instanceof MultiplayerServerListWidget.ServerEntry) {
                    ...
                    if (serverName != null) {
                        ...
                        this.client.setScreen(new ConfirmScreen(this::removeEntry, title, message, confirm, ScreenTexts.CANCEL));
                    }
                }
            })
        ...
        String editTexture = (currentEntry == null)
